...
io.on('connection', (socket) => {
  ...
  socket.on('call_next', () => {
    console.log('[call_next] received — waiting:', oldestWaiting()?.name, '| calling:', currentCalling()?.name);
    if (currentCalling()) {
  ...
  socket.on('admit_current', () => {
    ...
    if (!calling) {
      socket.emit('admit_error', { reason: 'No one is currently being called.' });
      return;
    }
...
function sendToArduinos(ticket) {
  ...
  for (const s of arduinoSockets) {
    try { s.write(msg); } catch (e) { /* socket may have closed */ }
  }
}
...
const tcpServer = net.createServer((socket) => {
  ...
  socket.on('end',   () => { arduinoSockets.delete(socket); console.log('TCP client disconnected'); });
  socket.on('error', (err) => { arduinoSockets.delete(socket); console.error('TCP error:', err.message); });
});
